...
const GitContent = () => {
	...
	useEffect(() => {
		axios.get("https://api.github.com/users/boidushya/repos?per_page=100")
		.then(res => {
			...
			let mappedData = [...base,...data.map(item => {
				return {
					name: item.name,
			...
			mappedData.forEach((item,idx) => {
				response += `<span class="${idx!==0&&`style2`}"><a target="_blank" href="${item.url}">${item.name}</a></span>${spaceList[idx]}<span class="${idx!==0&&`style5`}">${item.stars===0?"-":item.stars}</span>\t<span class="${idx!==0&&`style4`}">${item.description?item.description:""}</span>\n`
			});
			setData(response)
		})
	}, [])
